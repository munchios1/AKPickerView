...
- (void)setMaskDisabled:(BOOL)maskDisabled
{
    ...
    if (self.pickerImageType==AKCalander) {
        UIImageView * vw =[[UIImageView alloc]initWithFrame:CGRectMake(12,28, 35,35)];
        vw.image=[UIImage imageNamed:@"rcalander"];
        
        self.dateLabel= [[UILabel alloc]initWithFrame:CGRectMake(21, 40, 21, 21)];
        self.dateLabel.font =[UIFont fontWithName:@"Arial-BoldMT" size:15.0f];
        self.dateLabel.textColor=[UIColor orangeColor];
        self.dateLabel.text=@"11";
        [self.collectionView addSubview:self.dateLabel];
        [self.collectionView addSubview:vw];
    }
    if (self.pickerImageType==AKTimeClock) {
        UIImageView * vw =[[UIImageView alloc]initWithFrame:CGRectMake(12,10, 35,35)];
        vw.image=[UIImage imageNamed:@"rclock"];
        [self.collectionView addSubview:vw];
    }
...
#endif
    return CGSizeMake(75, ceilf(MAX(size.height, highlightedSize.height)));
}
...
-(NSString*)getFormatedDateForHours:(NSString*)hours{
    
    NSString *dateString = hours;
...
- (void)initialize
{
    ...
    self.monthLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.label.frame.origin.x,50, self.label.frame.size.width, 20)];
